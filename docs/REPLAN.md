> 2026-09-06：本文是历史决策记录；当前用户已重新确认需求与实现基线。被更新的设备清单、编辑隔离和后台同步语义以 [SPEC.md](SPEC.md) 为准。旧 SkillRiver 不恢复。
...
| D29 | 平台差异怎么处理 | **内含法，不是分支法。**skill 文件在所有机器逐字节相同，平台分支写进 skill 内容里（如 `platform/windows.md`），由读它的 AI 在运行时选择——AI 天然知道自己在什么系统上。**skillstow 为此需要改零行。**绝不允许往不同机器投不同版本的文件：投影一旦不等于真身，投影脏没脏的检测、本机活跃版本记录、回滚重组装立刻全部需要，那就是上一版 Bundle/Active/staged_bundle 的全部来源。这条不推翻 D7，恰恰是 D7 成立的原因。 | R9 |
| D30 | 按机器排除 | 本机配置加 `skip = ["md2hci"]`，被 skip 的 skill 在这台机器上一条链都不建。**不在共享清单里加「平台」维度**——你真正要表达的是「这台机器上不要」，平台只是代理变量（同样是 Windows，装了 pandoc 的那台就该有 md2hci）。把 OS 写进共享数据模型是四层 Profile 的第一块砖。代价：每台机器各写一次。 | R9 |
| D31 | Windows 建链方式（T0 实测后收敛） | **一律 `mklink /J`，不再试 `symlink_dir`。**实测：普通用户 junction 成功退出码 0，`symlink_dir` 报 1314 无特权，开发者模式注册表键缺失。D14「Junction 为主、零提权」**成立**。真符号链接对本程序零额外收益，保留那个分支等于每建一条链先失败一次系统调用（24 skill × 3 工具 = 72 次注定失败的调用）。 | R10 |
| D32 | 只用 remove_dir | **拆链只准 `remove_dir`，禁止 `remove_dir_all`。**实测两者对 junction 都安全（只删链、不动目标），但失败方向不同：万一 classify 判错把真实非空目录当成链，`remove_dir` 安全失败，`remove_dir_all` 删光用户数据。**会删东西的路径永远选 fail-safe 的那个。** | R10 |
| D33 | 真身仓禁止符号链接条目 | 仓库里不允许出现 git `120000` 模式条目；`init --import-from` 必须固化（deref）。实测：Windows 上 `core.symlinks` 默认 false，`120000` 会被检出成内容为目标路径的普通文本文件，skill 直接失效。**这条当场抓到真问题**：`~/skills/pua` 曾是指向 `~/.codex/pua/skills/pua` 的符号链接并被以 120000 提交，29 个文件 204 KB 实际没入库没备份，已在 `abb2ce5` 固化。上一版提交 `617741a` 标题同样叫「固化符号链接」——同一个坑第二次。 | R10 |
...
### 阶段 1.5 — T0 平台探针　✅ 已完成（2026-08-21）

- [x] 在真实 Windows（普通用户，rustc 1.98.0）验证四条关键假设
- [x] 结论写入 `docs/PLATFORM_NOTES.md`，并据此改动 SPEC 3.3 / 7.1 / 7.2、新增 2.5
- [x] 顺带抓到并修复 `pua` 符号链接未入库的问题（`abb2ce5`）
- 结果：三条假设成立，一条（先试 symlink_dir）被证伪并简化；
  另有两个原本没预料到的坑（junction 的 `is_dir()` 为 false、120000 条目退化成文本文件）
...
**2026-08-21 R9 更新**：D7 曾被质疑（「同一个 skill 在不同平台可能要不同」），
经查证后结论是 D7 成立——见 D29/D30，平台差异由 skill 内部承担，skillstow 不引入 OS 维度。

原判断保留如下：最可能被推翻的是 **D7（主轴是工具不是操作系统）**——如果哪天你真的有大量 skill
